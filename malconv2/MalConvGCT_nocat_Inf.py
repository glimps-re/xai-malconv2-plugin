...
from malconv2 import (
    checkpoint,
)  # This checkpoint implementation is faster than PyTorch's when using multiple GPUs


from malconv2.LowMemConvInf import LowMemConvBase
from malconv2.MalConvML_Inf import MalConvML
...
class MalConvGCT(LowMemConvBase):
    ...
    def forward(self, x):
        if self.low_mem:
            global_context = checkpoint.CheckpointFunction.apply(
                self.context_net.seq2fix, 1, x
            )
        else:
